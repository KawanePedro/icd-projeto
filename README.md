...
| `original_language` | Texto (String) | Idioma original do áudio do filme (código ISO). | `en, ko, pt` |
| `is_franchise` | Booleano (Bool) | Indica se o filme pertence a uma sequência/franquia cinematográfica. | `True ou False` |
| `budget` | Inteiro (Int) | Orçamento de produção do filme em dólares. Valores zerados indicam ausência de dados públicos oficiais. | `160000000` |
| `revenue` | Inteiro (Int) | Arrecadação total de bilheteria mundial em dólares. | `1132723226` |
| `metascore_critica` | Inteiro (Int) | Nota atribuída pela crítica especializada (Metacritic), variando de 0 a 100. Valores -1 indicam que o filme não possui nota oficial da crítica na plataforma. | `85` |